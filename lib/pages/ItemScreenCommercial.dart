...
import 'dart:convert';
import 'dart:io';
import 'package:aeroporto/pages/PieChartScreen.dart';
...
class ItemScrennCommercial extends StatefulWidget {
  final ItemCommercial item;

  const ItemScrennCommercial(this.item, {super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ItemScrennCommercialState createState() => _ItemScrennCommercialState();
}

class _ItemScrennCommercialState extends State<ItemScrennCommercial> {
  final List<PhotoDataCommercial> photoDataList = [];
  ...
  final List<MultiSelectItem<String>> _environmentItems = [
    ...
    MultiSelectItem('B1: BANHEIRO', 'B1'),
  ...
  Future<void> _savePhotos() async {
    ...
    await prefs.setString(
        '${widget.item.id}_commercial_photosData', jsonEncode(photosData));
  }

  Future<void> _loadPhotos() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? photosDataJson =
        prefs.getString('${widget.item.id}_commercial_photosData');

    if (photosDataJson != null) {
      List<dynamic> photosData = jsonDecode(photosDataJson);
      List<PhotoDataCommercial> loadedPhotos = [];

      for (var photoData in photosData) {
        loadedPhotos.add(PhotoDataCommercial(
          photoData['path'],
  ...
  Map<String, int> countDescriptionOccurrences(
      List<PhotoDataCommercial> photoDataList) {
            ...
            return AlertDialog(
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    setState(() {
                      photoDataList.add(PhotoDataCommercial(
                        imageFile.path,
