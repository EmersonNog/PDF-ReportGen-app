import 'dart:convert';
import 'package:aeroporto/pages/ItemScreenMixed.dart';
...
class ItemMixed {
  String id;
  String name;
  List<PhotoDataMixed> photos;

  ItemMixed(this.name, this.photos) : id = const Uuid().v4();
}

class PhotoDataMixed {
  String path;
  String description;
  String environment;

  PhotoDataMixed(this.path, this.description, this.environment);
}

class ReportPhotoMixed extends StatefulWidget {
  const ReportPhotoMixed({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ReportPhotoMixedState createState() => _ReportPhotoMixedState();
}

class _ReportPhotoMixedState extends State<ReportPhotoMixed> {
  List<ItemMixed> items = [];
  String searchText = '';
  ...
  Future<void> _saveItems() async {
    ...
    await prefs.setString('itemsDataMixed', jsonEncode(itemsData));
  }

  Future<void> _loadItems() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? itemsDataJson = prefs.getString('itemsDataMixed');

    if (itemsDataJson != null) {
      List<dynamic> itemsData = jsonDecode(itemsDataJson);
      List<ItemMixed> loadedItems = [];

      for (var itemData in itemsData) {
        List<PhotoDataMixed> photos =
            (itemData['photos'] as List<dynamic>).map((photoData) {
          return PhotoDataMixed(
            photoData['path'],
        ...
        loadedItems.add(ItemMixed(itemData['name'], photos)..id = itemData['id']);
      }
  ...
  Widget build(BuildContext context) {
    List<ItemMixed> filteredItems = _filterItems();
    return Scaffold(
      appBar: AppBar(
                                ...
                                child: ListTile(
                                  ...
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => ItemScrennMixed(
                                            filteredItems[index]),
                                      ),
                                    );
                                  },
