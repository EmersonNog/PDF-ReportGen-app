...
import 'dart:convert';
import 'dart:io';
import 'package:aeroporto/pages/Croqui.dart';
...
class ItemScreenInstitutional extends StatefulWidget {
  final ItemInstitutional item;

  const ItemScreenInstitutional(this.item, {super.key});

  @override
  _ItemScreenInstitutionalState createState() => _ItemScreenInstitutionalState();
}

class _ItemScreenInstitutionalState extends State<ItemScreenInstitutional> {
  final List<PhotoDataInstitutional> photoDataList = [];
  ...
  Future<void> _savePhotos() async {
    ...
    await prefs.setString(
        '${widget.item.id}_institutional_photosData', jsonEncode(photosData));
  }

  Future<void> _loadPhotos() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? photosDataJson =
        prefs.getString('${widget.item.id}_institutional_photosData');

    if (photosDataJson != null) {
      List<dynamic> photosData = jsonDecode(photosDataJson);
      List<PhotoDataInstitutional> loadedPhotos = [];

      for (var photoData in photosData) {
        loadedPhotos.add(PhotoDataInstitutional(
          photoData['path'],
  ...
  Map<String, int> countDescriptionOccurrences(List<PhotoDataInstitutional> photoDataList) {
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (context) => const ReportPhotoInstitutional()),
            );
          },
        ),
        actions: [
            ...
            return AlertDialog(
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    setState(() {
                      photoDataList.add(PhotoDataInstitutional(
                        imageFile.path,
