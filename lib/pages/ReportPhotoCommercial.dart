...
import 'ItemScreenCommercial.dart';
import 'home.dart';

class ItemCommercial {
  String id;
  String name;
  List<PhotoDataCommercial> photos;

  ItemCommercial(this.name, this.photos) : id = const Uuid().v4();
}

class PhotoDataCommercial {
  String path;
  String description;
  String environment;

  PhotoDataCommercial(this.path, this.description, this.environment);
}

class ReportPhotoCommercial extends StatefulWidget {
  const ReportPhotoCommercial({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ReportPhotoCommercialState createState() => _ReportPhotoCommercialState();
}

class _ReportPhotoCommercialState extends State<ReportPhotoCommercial> {
  List<ItemCommercial> items = [];
  String searchText = '';
  ...
  List<ItemCommercial> _filterItems() {
  ...
  Future<void> _addItem() async {
    ...
    if (itemName != null && itemName.isNotEmpty) {
      if (items.any((item) => item.name == itemName)) {
        ...
        setState(() {
          items.add(ItemCommercial(itemName, []));
          _saveItems();
        });
      }
    } else {
  ...
  Future<void> _saveItems() async {
    ...
    await prefs.setString('itemsDataCommercial', jsonEncode(itemsData));
  }

  Future<void> _loadItems() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? itemsDataJson = prefs.getString('itemsDataCommercial');

    if (itemsDataJson != null) {
      List<dynamic> itemsData = jsonDecode(itemsDataJson);
      List<ItemCommercial> loadedItems = [];

      for (var itemData in itemsData) {
        List<PhotoDataCommercial> photos =
            (itemData['photos'] as List<dynamic>).map((photoData) {
          return PhotoDataCommercial(
            photoData['path'],
        ...
        loadedItems.add(ItemCommercial(itemData['name'], photos)..id = itemData['id']);
      }
  ...
  Widget build(BuildContext context) {
    List<ItemCommercial> filteredItems = _filterItems();
    return Scaffold(
      appBar: AppBar(
                                ...
                                child: ListTile(
                                  ...
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => ItemScrennCommercial(
                                            filteredItems[index]),
                                      ),
                                    );
                                  },
